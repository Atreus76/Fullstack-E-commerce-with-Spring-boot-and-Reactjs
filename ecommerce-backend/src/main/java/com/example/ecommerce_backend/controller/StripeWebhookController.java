    ...
    public ResponseEntity<String> handle(@RequestBody String payload,
        ...
        try {
            ...
            if ("payment_intent.succeeded".equals(event.getType())) {
                ...
                if (intent != null) {
                    System.out.println("âœ… PaymentIntent parsed successfully (Unsafe Mode): " + intent.getId());
                    orderService.fulfillOrder(intent.getId());
                } else {
                    System.err.println("âŒ Failed to parse PaymentIntent even with unsafe deserialization.");
                }
            }
